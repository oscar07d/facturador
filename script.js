...
async function loadClientsIntoDropdown() {
    ...
    // Limpiar opciones anteriores y el array de clientes cargados
    customClientOptions.innerHTML = ''; 
    loadedClients = []; 

    // 1. Añadir siempre la opción "-- Nuevo Cliente --" PRIMERO a la lista de opciones
    ...
    // 2. Resetear el display principal si no hay un cliente ya seleccionado.
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            querySnapshot.forEach((docSnap) => {
                ...
                // --- Lógica para la Píldora de Estado de Última Factura del Cliente ---
                let estadoFacturaKey = (client.estadoUltimaFacturaCliente || "n/a").toLowerCase().replace(/ /g, '_');
                let estadoFacturaInfo = paymentStatusDetails[estadoFacturaKey] || paymentStatusDetails['n/a']; // Fallback
                
                // Construir el HTML para la opción con UNA SOLA píldora (estado de última factura)
        ...
        // Evitar añadir múltiples mensajes de error si ya existe uno
